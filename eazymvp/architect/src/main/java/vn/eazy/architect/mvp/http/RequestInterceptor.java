...
import vn.eazy.architect.mvp.utils.CharactorHandler;
import vn.eazy.architect.mvp.utils.ZipHelper;
...
public class RequestInterceptor implements Interceptor {
    ...
    public Response intercept(Chain chain) throws IOException {
        ...
        try {
            ...
            Timber.w("Http Error: " + ex);
        }

        long t2 = System.nanoTime();

        String bodySize = originalResponse.body().contentLength() != -1 ? originalResponse.body().contentLength()
                + "-byte" : "unknown-length";

        Timber.tag(getTag(request, "Request_Info")).w("Received response in [ %d-ms ] , [ %s ]%n%s"
